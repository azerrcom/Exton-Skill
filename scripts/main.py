...
def cmd_create_wallet(args):
    ...
    password = args.get("password") or os.environ.get("EXTON_KEY_PASSWORD")
    if not password:
        print(json.dumps({"error": "--password (или env EXTON_KEY_PASSWORD) обязателен для шифрования приватного ключа"}))
        return
...
def cmd_send(args):
    ...
    _password = os.environ.get("EXTON_KEY_PASSWORD")
    if not _password:
        print(json.dumps({"error": "EXTON_KEY_PASSWORD не задан — нужен пароль для расшифровки приватного ключа"}))
        return
    app_privkey = load_encrypted_key(_password)
        ...
        try:
            subprocess.run(
                ["openclaw", "message", "send",
                 ...
                 "--target", chat_id,
                 "--media", qr_url,
                 "--message", f"📱 {amount_nano / 1e9} GRAM → {to_raw[:20]}...\n"
